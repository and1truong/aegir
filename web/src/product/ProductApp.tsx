...
function EdgePrototypeBar({ stage, setStage, metrics }: { stage: EdgePrototypeStage; setStage: (stage: EdgePrototypeStage) => void; metrics?: EdgePrototypeMetrics }) {
  return <div className="flex min-h-8 items-center gap-1 border-b border-zinc-800 bg-cyan-950/10 px-3 text-[9px]"><span className="mr-1 font-semibold uppercase tracking-wider text-cyan-400">Edge lab · experimental</span>{edgePrototypeStages.map((item) => <button key={item.id} onClick={() => setStage(item.id)} title={`${item.id}. ${item.label}`} aria-pressed={stage === item.id} className={cn('rounded border px-1.5 py-0.5', stage === item.id ? 'border-cyan-700 bg-cyan-950/60 text-cyan-100' : 'border-zinc-800 text-zinc-500 hover:text-zinc-200')}>{item.id} · {item.label}</button>)}{metrics && <div className="ml-auto flex items-center gap-2 font-mono text-zinc-500"><span>crossings {metrics.crossingCount.baseline}→{metrics.crossingCount.prototype}</span><span>direction proxy {Math.round(metrics.directionRecognition * 100)}%</span><span>click proxy {Math.round(metrics.clickAccuracy * 100)}%</span><span>focus/path proxy {Math.round(metrics.focalPathComprehension * 100)}%</span><span>{metrics.layoutTimeMs.toFixed(1)}ms</span><span className={metrics.routingDecision === 'retain-dagre' ? 'text-emerald-400' : 'text-amber-300'}>{metrics.routingDecision === 'retain-dagre' ? 'Dagre retained' : 'Evaluate ELK'}</span></div>}</div>;
}
...
function Explorer({ theme, focusMode, setFocusMode }: GraphViewProps) {
  ...
  const [showPrivateSymbols, setShowPrivateSymbols] = useState(false);
  ...
  const filtered = filterSymbols(modeNodes, query, showPrivateSymbols).slice(0, 300);
  const selectGraphNode = (id: string) => {
    const aggregate = graph.aggregates.find((item) => item.nodeId === id);
    if (aggregate) {
      dispatch({ type: 'toggleFrontier', frontierId: aggregate.branchKey, beyondDepth: !aggregate.withinDepth });
      return;
    }
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      {!focusMode && <EdgePrototypeBar stage={edgePrototypeStage} setStage={setEdgePrototypeStage} metrics={edgePrototypeMetrics} />}
      ...
      <div className="flex min-h-0 flex-1">
        {!focusMode && <aside className="flex w-[250px] shrink-0 flex-col border-r border-zinc-800">
          <div className="flex items-center gap-1.5 border-b border-zinc-800 p-2"><div className="flex h-7 min-w-0 flex-1 items-center gap-2 rounded-md border border-zinc-800 bg-zinc-950 px-2"><Search className="h-3.5 w-3.5 shrink-0 text-zinc-600" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Filter symbols…" className="min-w-0 w-full bg-transparent font-mono text-[11px] outline-none" /></div><button type="button" aria-pressed={showPrivateSymbols} title={showPrivateSymbols ? 'Hide private symbols' : 'Show private symbols'} onClick={() => setShowPrivateSymbols((value) => !value)} className={cn('h-7 shrink-0 rounded-md border px-2 text-[9.5px]', showPrivateSymbols ? 'border-sky-800 bg-sky-950/40 text-sky-200' : 'border-zinc-800 text-zinc-500 hover:text-zinc-200')}>Private</button></div>
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div className={cn('grid min-h-0 flex-1', !focusMode && inspectorOpen ? 'grid-cols-[1fr_360px]' : 'grid-cols-1')}>
        <div className="relative min-w-0"><SystemGraph nodes={graph.nodes} edges={graph.edges} frontiers={graph.aggregates} decor={decor} selected={reviewProjectedSelected} selectedEdge={selectedEdgeId} onSelect={selectGraphNode} onEdgeSelect={(id) => dispatch({ type: 'selectEntity', entity: { kind: 'edge', id } })} anchorNodeId={reviewProjectedSelected ?? graph.visibleGraph.rootNodeIds[0]} pinnedNodeIds={reviewProjectedPins} topologyRevision={graph.visibleGraph.revision} layoutStrategy="review-LR" edgePrototypeStage={edgePrototypeStage} deltaEdgeIds={new Set(Object.keys(decor.edgeLabel ?? {}))} minimap theme={theme} />{focusMode && <button onClick={() => setFocusMode(false)} aria-label="Exit focus mode" className="absolute right-3 top-3 z-20 inline-flex items-center gap-1.5 rounded-md border border-zinc-700 bg-zinc-950/90 px-2.5 py-1.5 text-[11px] text-zinc-300 shadow-lg hover:bg-zinc-800"><Minimize2 className="h-3.5 w-3.5" /> Exit focus</button>}</div>
