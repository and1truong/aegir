...
function Explorer({ theme, focusMode, setFocusMode }: GraphViewProps) {
  ...
  const abstractionGraph = useMemo(() => abstractGraph(canonicalGraphIndex, investigation.abstraction), [canonicalGraphIndex, investigation.abstraction]);
  ...
  }), [graphIndex, mode, projectedSelected, lintRoots, upstreamDepth, downstreamDepth, enabledRelationships, branchExpansions, investigation.evidencePolicy]);
...
function ReviewScreen({ theme, focusMode, setFocusMode }: GraphViewProps) {
  ...
  const graph = projectPRGraphIndex(reviewIndex, { projectionId: 'review', activeNodeId: reviewPolicy === 'blast-radius' ? reviewProjectedSelected : undefined, upstreamDepth, downstreamDepth, edgeKinds: enabledRelationships, branchExpansions, nodeBudget: 30, evidencePolicy: investigation.evidencePolicy });
  const selectedEdgeId = investigation.selectedEntity?.kind === 'edge' ? investigation.selectedEntity.id : undefined;
  const selectedEdge = graph.edges.find((edge) => edge.id === selectedEdgeId);
  const selectedVisibleEdge = graph.visibleGraph.edges.find((edge) => edge.kind === 'real' && edge.id === selectedEdgeId);
  const selectedNodeDelta = delta.nodes.find((entry) => entry.id === selected || (reviewProjectedSelected ? reviewAbstractionGraph.representativeNodeMembers.get(reviewProjectedSelected)?.includes(entry.id) : false));
  const selectedEdgeDelta = delta.edges.find((entry) => entry.id === selectedEdgeId || (selectedEdgeId ? reviewAbstractionGraph.representativeEdgeMembers.get(selectedEdgeId)?.includes(entry.id) : false));
  useEffect(() => {
    if (selectedEdgeId && !selectedEdge) dispatch({ type: 'selectEntity', entity: selected ? { kind: 'node', id: selected } : null });
  }, [selectedEdgeId, selectedEdge, selected, dispatch]);
  useEffect(() => {
