...
export function AttentionOverview({ openUnit, theme }: { openUnit: (unitId: string) => void; theme: 'light' | 'dark' }) {
  ...
  }, [windowDays, query, region, scope, selectedUnitID]);
  ...
  const units = useMemo(() => (landscape?.units ?? []).filter((unit) => (region === 'all' || unit.region === region) && (scope === 'all' || subsystem(unit) === scope) && (!query.trim() || `${unit.unit.label} ${unit.unit.path ?? ''}`.toLowerCase().includes(query.toLowerCase()))), [landscape, query, region, scope]);
