  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div className="flex min-h-0 flex-1">
        ...
        <main className="relative min-w-0 flex-1"><SystemGraph nodes={graph.nodes} edges={graph.edges} frontiers={graph.aggregates} decor={decor} selected={projectedSelected} selectedEdge={selectedEdgeId} onSelect={selectGraphNode} onEdgeSelect={(id) => dispatch({ type: 'selectEntity', entity: { kind: 'edge', id } })} onDoubleClick={expandGraphNode} anchorNodeId={projectedSelected ?? graph.visibleGraph.rootNodeIds[0]} pinnedNodeIds={projectedPins} lockedPathNodeIds={investigation.lockedPath?.nodeIds} topologyRevision={graph.visibleGraph.revision} layoutStrategy={projectionDefinitions[mode].layoutStrategy} edgePrototypeStage={edgePrototypeStage} pathEdgeIds={activePathEdgeIds} onEdgePrototypeMetrics={(next) => setEdgePrototypeMetrics((current) => current && current.crossingCount.baseline === next.crossingCount.baseline && current.crossingCount.prototype === next.crossingCount.prototype && current.directionRecognition === next.directionRecognition && current.clickAccuracy === next.clickAccuracy && current.focalPathComprehension === next.focalPathComprehension && current.layoutTimeMs === next.layoutTimeMs && current.routingDecision === next.routingDecision ? current : next)} minimap theme={theme} />{focusMode && <button onClick={() => setFocusMode(false)} aria-label="Exit focus mode" className="absolute right-3 top-3 z-20 inline-flex items-center gap-1.5 rounded-md border border-zinc-700 bg-zinc-950/90 px-2.5 py-1.5 text-[11px] text-zinc-300 shadow-lg hover:bg-zinc-800"><Minimize2 className="h-3.5 w-3.5" /> Exit focus</button>}</main>
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div className={cn('grid min-h-0 flex-1', !focusMode && inspectorOpen ? 'grid-cols-[1fr_360px]' : 'grid-cols-1')}>
        <div className="relative min-w-0"><SystemGraph nodes={graph.nodes} edges={graph.edges} frontiers={graph.aggregates} decor={decor} selected={reviewProjectedSelected} selectedEdge={selectedEdgeId} onSelect={selectGraphNode} onEdgeSelect={(id) => dispatch({ type: 'selectEntity', entity: { kind: 'edge', id } })} onDoubleClick={selectGraphNode} anchorNodeId={reviewProjectedSelected ?? graph.visibleGraph.rootNodeIds[0]} pinnedNodeIds={reviewProjectedPins} topologyRevision={graph.visibleGraph.revision} layoutStrategy="review-LR" edgePrototypeStage={edgePrototypeStage} deltaEdgeIds={new Set(Object.keys(decor.edgeLabel ?? {}))} minimap theme={theme} />{focusMode && <button onClick={() => setFocusMode(false)} aria-label="Exit focus mode" className="absolute right-3 top-3 z-20 inline-flex items-center gap-1.5 rounded-md border border-zinc-700 bg-zinc-950/90 px-2.5 py-1.5 text-[11px] text-zinc-300 shadow-lg hover:bg-zinc-800"><Minimize2 className="h-3.5 w-3.5" /> Exit focus</button>}</div>
