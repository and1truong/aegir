...
func TestRunConnectsEndpointToInjectedMethodCallback(t *testing.T) {
	...
	for _, label := range []string{"GET /ranged", "GET /ranged-literal", "GET /ranged-constructor", "GET /ranged-named", "GET /ranged-generic", "GET /ranged-generic-constructor"} {
		if connections[label] != handlerIDs["AdminHandler.Handle"] {
			t.Fatalf("range-bound receiver for %q was not connected to AdminHandler.Handle: %#v", label, snapshot.Edges)
		}
	}
	if len(endpointIDs) != 22 || endpointIDs["GET /config"] != "" || endpointIDs["GET /config-field"] != "" {
