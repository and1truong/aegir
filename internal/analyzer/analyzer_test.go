...
func TestRunConnectsEndpointToInjectedMethodCallback(t *testing.T) {
	...
	for path, body := range map[string]string{
		...
		"routes/z_test.go":   "package routes_test\nimport (routes \"example.com/routes/routes\"; \"github.com/labstack/echo/v4\")\ntype LocalHandler struct{}\nfunc (*LocalHandler) Handle() {}\nfunc NewHandler[T any]() *LocalHandler { return &LocalHandler{} }\nfunc onMiss() {}\nfunc TestExternal(e *echo.Echo, h *routes.LocalHandler) { e.GET(\"/external-test\", h.Handle); own := NewHandler[int](); e.GET(\"/external-test-own\", own.Handle) }\n",
	} {
		full := filepath.Join(root, path)
		if err := os.MkdirAll(filepath.Dir(full), 0o755); err != nil {
			t.Fatal(err)
	...
	for _, label := range []string{"GET /ranged", "GET /ranged-literal", "GET /ranged-constructor", "GET /ranged-named", "GET /ranged-generic"} {
		if connections[label] != handlerIDs["AdminHandler.Handle"] {
