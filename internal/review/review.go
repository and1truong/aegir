...
func snapshotFingerprint(snapshot analyzer.Snapshot) string {
	...
	for _, edge := range edges {
		...
		hash.Write([]byte{0})
		hash.Write([]byte(edgeFingerprint(edge)))
		hash.Write([]byte{0})
	}
	analysisValue := snapshot.Analysis
	analysisValue.Rules = append([]analyzer.Rule(nil), analysisValue.Rules...)
	analysisValue.Violations = append([]analyzer.Violation(nil), analysisValue.Violations...)
	analysisValue.Coverage = append([]analyzer.Coverage(nil), analysisValue.Coverage...)
	analysisValue.Contracts = append([]analyzer.Contract(nil), analysisValue.Contracts...)
	analysisValue.Complexity = append([]analyzer.Complexity(nil), analysisValue.Complexity...)
	analysisValue.Telemetry = append([]analyzer.Telemetry(nil), analysisValue.Telemetry...)
	sort.Slice(analysisValue.Rules, func(i, j int) bool { return analysisValue.Rules[i].ID < analysisValue.Rules[j].ID })
	sort.Slice(analysisValue.Violations, func(i, j int) bool { return analysisValue.Violations[i].ID < analysisValue.Violations[j].ID })
	sort.Slice(analysisValue.Coverage, func(i, j int) bool { return analysisValue.Coverage[i].NodeID < analysisValue.Coverage[j].NodeID })
	sort.Slice(analysisValue.Contracts, func(i, j int) bool { return analysisValue.Contracts[i].ID < analysisValue.Contracts[j].ID })
	sort.Slice(analysisValue.Complexity, func(i, j int) bool { return analysisValue.Complexity[i].NodeID < analysisValue.Complexity[j].NodeID })
	sort.Slice(analysisValue.Telemetry, func(i, j int) bool { return analysisValue.Telemetry[i].NodeID < analysisValue.Telemetry[j].NodeID })
	evidenceValue := append([]analyzer.EvidenceRecord(nil), snapshot.Evidence...)
	sort.Slice(evidenceValue, func(i, j int) bool { return evidenceValue[i].ID < evidenceValue[j].ID })
	analysis, _ := json.Marshal(analysisValue)
	evidence, _ := json.Marshal(evidenceValue)
	hash.Write(analysis)
	hash.Write([]byte{0})
	hash.Write(evidence)
